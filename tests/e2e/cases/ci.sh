...
function test::ci::_start() {
  ...
  testenv::k8s::kind::load_image datenlord/xline-operator:latest >/dev/null 2>&1
...
function test::run::ci::basic_validation() {
  ...
  test::ci::_etcdctl_expect "$endpoints" "put A 1" "OK"
  test::ci::_etcdctl_expect "$endpoints" "get A" "A\n1"
  endpoints=$(test::ci::_mk_endpoints 1)
  test::ci::_etcdctl_expect "$endpoints" "put A 2" "OK"
  test::ci::_etcdctl_expect "$endpoints" "get A" "A\n2"
}
...
function test::run::ci::scale_validation() {
  ...
  test::ci::_scale_cluster 5
  endpoints=$(test::ci::_mk_endpoints 5)
  test::ci::_etcdctl_expect "$endpoints" "put A 1" "OK"
  test::ci::_etcdctl_expect "$endpoints" "get A" "A\n1"

  test::ci::_scale_cluster 3
